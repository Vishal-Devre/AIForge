  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Live" value={deployments.filter(d => d.status === 'deployed').length.toString()} icon={<Globe className="h-5 w-5" />} trend={{ value: 100, positive: true }} />
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            description="Connect a GitHub repository to deploy your first application"
          ...
          filtered.map((dep, i) => {
            ...
            return (
              ...
              >
                <Card className="hover:border-primary-500/20 transition-all duration-200">
                  <CardContent className="p-5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        <div className={`h-10 w-10 rounded-xl ${getStatusBg(dep.status)} flex items-center justify-center`}>
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            {dep.url && (
                              <a href="#" className="text-xs text-primary-400 hover:text-primary-300 flex items-center gap-1">
                                {dep.url} <ArrowUpRight className="h-3 w-3" />
                              </a>
                            )}
                            <span className="text-xs text-slate-500">{dep.region}</span>
                            <span className="text-xs text-slate-500">&middot;</span>
                            <GitBranch className="h-3 w-3 text-slate-500" />
                            <span className="text-xs text-slate-500">{dep.branch}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="hidden md:flex items-center gap-6 mx-6">
                        <div className="text-right">
                          ...
                          <p className="text-sm font-medium text-white">{dep.cpu} cores</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-slate-400">Memory</p>
                          <p className="text-sm font-medium text-white">{dep.memory} GB</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-slate-400">Commits</p>
                          <p className="text-sm font-medium text-white">{dep.commits}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        <span className="text-xs text-slate-500 hidden sm:block">{timeAgo(dep.deployedAt)}</span>
                        <DropdownMenu>
